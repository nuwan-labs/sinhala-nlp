...
UNIT_WORDS = {
    # Indian system
    'තෝලා',       # 12 g
    'තෝල',        # alternate spelling of තෝලා
    ...
    'මංචාඩි',     # 250 mg
    'කලං',        # 5 g
    'කලන්',       # alternate spelling of කලං
    'අඩපලං',      # 30 g
    'පලං',        # 60 g  (same as පල)
    'ප්‍රසෘති',   # 120 g
    'අර්ධශරාව',   # 240 g
    'ශරාව',       # 480 g
    'ප්‍රස්ථ',    # 960 g
    'නැලි',       # = ප්‍රස්ථ (960 g / 960 ml)
    'ආඪක',        # 3840 g
    'ද්‍රෝණ',     # 15360 g
    'තුලා',       # 6000 g  (=100 palam)
    'පලම්',       # = පල / පලං variant
    # Volume / liquid units
    'ලීටර්',      # litre (volume equivalent, like ග්‍රෑ for solids)
    'ලී',         # abbreviated litre
    'සේරු',       # traditional liquid measure
    'පත',         # liquid measure (= අර්ධශරාව)
    'කුඩව',       # small measure
    'බෝ',         # traditional measure
    'බෝතල',       # bottle measure
    # Imperial / colonial units
    'රාත්තල්',    # pound (British era)
    'ග්‍රෑම්ස්',  # grams (English loanword variant)
    # Time-based dosage units that appear as quantities
    'රා',         # රා. abbreviation (used in some formulas)
    # Teaspoon (two tokens merged before detection)
    'තේ හැඳි',    # teaspoon
}
...
def parse_ingredient(cw):
    ...
    Returns None if cw is empty.
    Returns (text, None, None, None, None) if no unit found.

    grams:  the number following a ග්‍රෑ / ග්‍රෑම් keyword
    litres: the number following a ලී / ලීටර් keyword
    Both are treated identically — the field name reflects the physical quantity.
    """
    ...
    for i, t in enumerate(tail):
        ...
        if t in ('ලී', 'ලීටර්') or t.startswith('ලී'):
            for g in tail[i + 1:]:
                if _GRM_RE.match(g): litres = g; break
            break
...
def _is_inline_instruction(group_text, row_words):
    ...
    # Explicit "equal-parts" instruction tokens — always an instruction
    # regardless of position or punctuation.
    _EQUAL_PARTS = frozenset({
        'සමභාගව', 'සමබාගව', 'සමභාගවයි',           # "in equal parts"
        'මදට', 'මදටිය',                             # "manchadi each" (dosage note)
    })
    if any(tok in _EQUAL_PARTS for tok in words):
        return True

    # Sentence-like text: ends with punctuation AND is long or contains "යුතු"
        ...
        if state == 'ENTRY_HEADER':
            ...
            if wt(first_cw) in UNIT_WORDS and wx(first_cw) > SHARED_QTY_X:
                ...
                if unit is None:
                    ...
                    if name:
                        current['යෝගය'].append(_make_ing(name, unit, amount, grams, litres))
